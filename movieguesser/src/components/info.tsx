...
const Info = (props: any) => {
  ...
  return <div className="flex flex-col items-center h-[32rem] relative w-full">
    <div className="text-xl">How to play</div>
    <div className="w-full h-8">&nbsp;</div>
    <div className="w-full">
      {page === 0 &&
        <div>
          <span>Try to guess the movie, while revealing as few clues as possible.</span>
          <div className="w-full h-4">&nbsp;</div>
          <div className="flex">
            <div className="grow">
              <span className="inline-block">Press the clue to the right to reveal it...</span>
              <span className={`mt-4 inline-block transition-all ${actorReveals >= 1 ? "opacity-100" : "opacity-0"}`}>Some clues can be pressed several times to reveal more information.</span>
              <span className={`mt-4 inline-block transition-all ${actorReveals >= 1 ? "opacity-100" : "opacity-0"}`}>Now, press the clue again.</span>
            </div>
            <div className="">
              <div className="h-48 aspect-[1/1.3] ml-2">
                <Clue
                  clue="actor1"
          ...
          <span className={`mt-4 inline-block transition-all ${actorReveals == 2 ? "opacity-100" : "opacity-0"}`}>When a clue has a green background, there is no more information to be revealed.</span>
          <span className={`mt-8 inline-block transition-all ${actorReveals == 2 ? "opacity-100" : "opacity-0"}`}>Click the <AiOutlineArrowRight style={{ display: "inline" }} />-button below to go to the next page.</span>
        </div>
      }

      {page === 1 && <>
        <div className="w-full">
          <span>You start the game with 100 points, but as you reveal clues your score will decrease.</span>
        </div>
        ...
        <div className="w-full">
          <span>Each clue has a point cost associated with it. Hover over the title clue below to view its cost.</span>
          <div className="w-full h-2">&nbsp;</div>
          <span>Reveal the title clue and watch your point total decrease.</span>
        </div>

        <div className="w-full h-2">&nbsp;</div>
        <div className="h-32 w-full flex-center">
          <Clue
            clue="title"
            ...
            onReveal={(reveals) => { setTitleReveals(reveals); setPoints(prev => prev + clueSpecification["title"].pointCost[reveals - 1]) }}
      ...
      {page === 2 && <>
        ...
        <GuessInputLine isTutorial={true} onGuess={(guess: string) => { setWon(guess === props.tutorialMovieInfo.title); setGuesses(prev => prev + 1) }} movieTitle={props.tutorialMovieInfo.title} win={won} score={100 - points} guesses={guesses} />


        <div className="w-full">
          <span className="text-sm">Hint: the name of the movie is &nbsp;
            <span className="transition-all bg-black text-black hover:bg-transparent hover:text-inherit">John Wick</span>
          </span>
        </div>
